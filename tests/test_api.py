...
import uwlight
from uwlight import MissingQuantityError, ProvenanceWarning, Water
...
def test_missing_values_stay_missing():
    ...
    w = uwlight.water("3C", source="solonenko2015")
...
def test_flagged_wavelengths_warn():
    ...
    w = uwlight.water("IA", source="solonenko2015")
...
def test_sound_wavelengths_do_not_warn():
    w = uwlight.water("9C", source="solonenko2015")
    with warnings.catch_warnings():
...
def test_kd_spectrum_reproduces_austin_table6():
    ...
    predicted = uwlight.kd_spectrum(k475, 475, wl)
...
def test_b_from_c_matches_the_ratio_table():
    ...
    bw, cw = 0.0019, 0.0659
    c = 0.5
    b = uwlight.b_from_c(c, 555, bw=bw, cw=cw)
    assert b == pytest.approx((c - cw) * 0.904 + bw, rel=1e-6)

    lower = uwlight.b_from_c(c, 555, bw=bw, cw=cw, bound="min")
    upper = uwlight.b_from_c(c, 555, bw=bw, cw=cw, bound="max")
    assert lower < b < upper


def test_sources_are_described():
    for key, source in uwlight.SOURCES.items():
